...
from sqlalchemy import AsyncAdaptedQueuePool, select, update, ScalarResult
...
class Info(Base):
    ...
    sheet_id: Mapped[str] = mapped_column(__type="text", nullable=True)

    cron_question_about_game: Mapped[str] = mapped_column(__type="varchar(40)", nullable=True)
    ...
    range: Mapped[str] = mapped_column(__type="varchar(9)", nullable=True)
